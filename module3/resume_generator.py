...
def ask_ollama(mode: str, prompt_text: str) -> str:
    ...
    if mode == "local":
        ...
        if not api_key:
            raise ValueError(
                "OLLAMA_API_KEY is missing. Set it before running cloud mode."
            )
...
# Local resume quality: The local resume was acceptable but simpler; it followed the basic structure, but the styling, spacing, typography, and two-column layout were less polished.
# Cloud resume quality: The cloud resume was better overall, including styling; it produced a more professional two-column layout, cleaner spacing, stronger visual hierarchy, better section headings, and a more polished resume design.